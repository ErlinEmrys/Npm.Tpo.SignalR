...
describe( "JsonHubProtocol", () =>
{
	it( "can write/read non-blocking Invocation message", async () =>
	{
		...
		{
			const invocation = {
				arguments: [ 42, true, "test", [ "x1", "y2" ], null ], headers: {}, target: "myMethod", type: MessageType.Invocation,
	...
	it( "can read Invocation message with Date argument", async () =>
	{
		...
		{
			const invocation = {
				arguments: [ Date.UTC( 2018, 1, 1, 12, 34, 56 ) ], headers: {}, target: "mymethod", type: MessageType.Invocation,
			} as InvocationMessage;
	...
	it( "can write/read Invocation message with headers", async () =>
	{
		...
		{
			const invocation = {
				arguments: [ 42, true, "test", [ "x1", "y2" ], null ], headers: {
					foo: "bar",
				}, streamIds: [], target: "myMethod", type: MessageType.Invocation,
			} as InvocationMessage;
	...
	it( "can write/read Invocation message", async () =>
	{
		...
		{
			const invocation = {
				arguments: [ 42, true, "test", [ "x1", "y2" ], null ], headers: {}, invocationId: "123", target: "myMethod", type: MessageType.Invocation,
			} as InvocationMessage;
	...
	( [ [ `{"type":3, "invocationId": "abc", "error": "Err", "result": null, "headers": {}}${ TextMessageFormat.RecordSeparator }`, {
		error: "Err", headers: {}, invocationId: "abc", result: null, type: MessageType.Completion,
	} as CompletionMessage ], [ `{"type":3, "invocationId": "abc", "result": "OK", "headers": {}}${ TextMessageFormat.RecordSeparator }`, {
		headers: {}, invocationId: "abc", result: "OK", type: MessageType.Completion,
	} as CompletionMessage ], [ `{"type":3, "invocationId": "abc", "result": null, "headers": {}}${ TextMessageFormat.RecordSeparator }`, {
		headers: {}, invocationId: "abc", result: null, type: MessageType.Completion,
	} as CompletionMessage ], [ `{"type":3, "invocationId": "abc", "result": 1514805840000, "headers": {}}${ TextMessageFormat.RecordSeparator }`, {
		headers: {}, invocationId: "abc", result: Date.UTC( 2018, 0, 1, 11, 24, 0 ), type: MessageType.Completion,
	} as CompletionMessage ], [ `{"type":3, "invocationId": "abc", "result": null, "headers": {}, "extraParameter":"value"}${ TextMessageFormat.RecordSeparator }`, {
		extraParameter: "value", headers: {}, invocationId: "abc", result: null, type: MessageType.Completion,
	} as CompletionMessage ] ] as [ string, CompletionMessage ][] ).forEach( ( [ payload, expectedMessage ] ) => it( "can read Completion message", async () =>
	{
		await VerifyLogger.run( async ( logger ) =>
		{
	...
	( [ [ `{"type":2, "invocationId": "abc", "headers": {}, "item": 8}${ TextMessageFormat.RecordSeparator }`, {
		headers: {}, invocationId: "abc", item: 8, type: MessageType.StreamItem,
	} as StreamItemMessage ], [ `{"type":2, "invocationId": "abc", "headers": {}, "item": 1514805840000}${ TextMessageFormat.RecordSeparator }`, {
		headers: {}, invocationId: "abc", item: Date.UTC( 2018, 0, 1, 11, 24, 0 ), type: MessageType.StreamItem,
	} as StreamItemMessage ] ] as [ string, StreamItemMessage ][] ).forEach( ( [ payload, expectedMessage ] ) => it( "can read StreamItem message", async () =>
	{
		await VerifyLogger.run( async ( logger ) =>
		{
			const messages = new JsonHubProtocol().parseMessages( payload, logger );
	...
	( [ [ `{"type":2, "invocationId": "abc", "headers": {"t": "u"}, "item": 8}${ TextMessageFormat.RecordSeparator }`, {
		headers: {
			t: "u",
		}, invocationId: "abc", item: 8, type: MessageType.StreamItem,
	} as StreamItemMessage ] ] as [ string, StreamItemMessage ][] ).forEach( ( [ payload, expectedMessage ] ) => it( "can read message with headers", async () =>
	{
		await VerifyLogger.run( async ( logger ) =>
		{
			const messages = new JsonHubProtocol().parseMessages( payload, logger );
	...
	( [ [ "message with empty payload", `{}${ TextMessageFormat.RecordSeparator }`, new Error( "Invalid payload." ) ],
		...
		[ "Completion message with non-string error", `{"type":3,"invocationId":"1","error":21}${ TextMessageFormat.RecordSeparator }`, "Invalid payload for Completion message." ] ] as [ string, string, string ][] ).forEach( ( [ name, payload, expectedError ] ) => it( "throws for " + name, async () =>
	{
		await VerifyLogger.run( async ( logger ) =>
		{
	...
	it( "can write/read Ack message", async () =>
	{
		...
		{
			const ackMessage = {
				sequenceId: 45, type: MessageType.Ack,
			} as AckMessage;
	...
	it( "can write/read Sequence message", async () =>
	{
		await VerifyLogger.run( async ( logger ) =>
		{
			const sequenceMessage = {
				sequenceId: 46, type: MessageType.Sequence,
			} as SequenceMessage;
