...
describe( "HubConnection", () =>
{
		...
		it( "sends a non blocking invocation", async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				try
				{
					...
					expect( JSON.parse( connection.sentData[ 0 ] ) ).toEqual( {
						arguments: [ "arg", 42 ], target: "testMethod", type: MessageType.Invocation,
					} );
				}
				finally
				{
		...
		it( "works if argument is null", async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				try
				{
					...
					expect( connection.sentData.length ).toBe( 1 );
					expect( JSON.parse( connection.sentData[ 0 ] ) ).toEqual( {
						arguments: [ "arg", null ], target: "testMethod", type: MessageType.Invocation,
					} );
				}
				finally
				{
		...
		it( "sends an invocation", async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				try
				{
					...
					expect( connection.sentData.length ).toBe( 1 );
					expect( JSON.parse( connection.sentData[ 0 ] ) ).toEqual( {
						arguments: [ "arg", 42 ], invocationId: connection.lastInvocationId, target: "testMethod", type: MessageType.Invocation,
					} );
		...
		it( "is able to send stream items to server with invoke", async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				try
				{
					...
					expect( JSON.parse( connection.sentData[ 2 ] ) ).toEqual( {
						arguments: [ "arg" ], invocationId: "1", streamIds: [ "0" ], target: "testMethod", type: MessageType.Invocation,
					} );
					...
					expect( JSON.parse( connection.sentData[ 3 ] ) ).toEqual( {
						invocationId: "0", item: "item numero uno", type: MessageType.StreamItem,
					} );
		...
		it( "is able to send stream items to server with send", async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				try
				{
					...
					expect( JSON.parse( connection.sentData[ 2 ] ) ).toEqual( {
						arguments: [ "arg" ], streamIds: [ "0" ], target: "testMethod", type: MessageType.Invocation,
					} );
		...
		it( "is able to send stream items to server with stream", async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				try
				{
					...
					expect( JSON.parse( connection.sentData[ 2 ] ) ).toEqual( {
						arguments: [ "arg" ], invocationId: "1", streamIds: [ "0" ], target: "testMethod", type: MessageType.StreamInvocation,
					} );
		...
		it( "invocations ignored in callbacks not registered", async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				try
				{
					...
					connection.receive( {
						arguments: [ "test" ], nonblocking: true, target: "message", type: MessageType.Invocation,
					} );
		...
		it( "invocations ignored in callbacks that have registered then unregistered", async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				try
				{
					...
					connection.receive( {
						arguments: [ "test" ], invocationId: "0", nonblocking: true, target: "message", type: MessageType.Invocation,
					} );

					expect( warnings ).toEqual( [ "No client method with the name 'message' found.", "No result given for 'message' method and invocation ID '0'." ] );
				}
				finally
				{
					await hubConnection.stop();
		...
		it( "all handlers can be unregistered with just the method name", async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				try
				{
					...
					connection.receive( {
						arguments: [], nonblocking: true, target: "inc", type: MessageType.Invocation,
					} );
		...
		it( "callback invoked when server invokes a method on the client and then handles rejected promise on send", async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				try
				{
					...
					connection.receive( {
						arguments: [ "test" ], nonblocking: true, target: "message", invocationId: "0", type: MessageType.Invocation,
					} );
		...
		it( "stop on error close message", async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				try
				{
					...
					connection.receive( {
						allowReconnect: true, error: "Error!", type: MessageType.Close,
					} );
		...
		it( "can have multiple callbacks", async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				try
				{
					...
					connection.receive( {
						arguments: [], nonblocking: true, target: "message", type: MessageType.Invocation,
					} );
		...
		it( "unsubscribing dynamically doesn't affect the current invocation loop", async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				try
				{
					...
					connection.receive( {
						arguments: [], nonblocking: true, target: eventToTrack, type: MessageType.Invocation,
					} );
		...
		it( "using null/undefined for methodName or method no-ops", async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				try
				{
					...
					connection.receive( {
						arguments: [], invocationId: "0", nonblocking: true, target: "message", type: MessageType.Invocation,
					} );
		...
		it( "can return result from callback", async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				try
				{
					...
					connection.receive( {
						arguments: [], invocationId: "1", nonblocking: true, target: "message", type: MessageType.Invocation,
					} );
		...
		it( "logs error if return result not expected", async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				try
				{
					...
					connection.receive( {
						arguments: [], invocationId: undefined, nonblocking: true, target: "message", type: MessageType.Invocation,
					} );
	...
	describe( "stream", () =>
	{
		it( "sends an invocation", async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				try
				{
					...
					expect( connection.sentData.length ).toBe( 3 );
					expect( JSON.parse( connection.sentData[ 2 ] ) ).toEqual( {
						arguments: [ "arg", 42 ], invocationId: connection.lastInvocationId, target: "testStream", type: MessageType.StreamInvocation,
					} );
				}
				finally
				{
		...
		it( "can be canceled", async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				try
				{
					...
					expect( JSON.parse( connection.sentData[ 3 ] ) ).toEqual( {
						invocationId: connection.lastInvocationId, type: MessageType.CancelInvocation,
					} );
				}
				finally
				{
		...
		it( "ignores error from 'next' and 'complete' function", async () =>
		{
			...
			}, /Stream callback threw error: Error: from complete/, /Stream callback threw error: Error: from next/ );
		} );
		...
		it( "sends sequence message on reconnect", async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				try
				{
					...
					expect( connection.sentData.length ).toBe( 3 );
					expect( JSON.parse( connection.sentData[ 2 ] ) ).toEqual( {
						type: MessageType.Sequence, sequenceId: 1,
					} );
				}
				finally
				{
			...
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				try
				{
					...
					expect( JSON.parse( connection.sentData[ 6 ] ) ).toEqual( {
						type: MessageType.Invocation, target: "test", arguments: [ 13 ],
					} );
					expect( JSON.parse( connection.sentData[ 7 ] ) ).toEqual( {
						type: MessageType.Invocation, target: "test", arguments: [ 12 ],
					} );
					expect( JSON.parse( connection.sentData[ 8 ] ) ).toEqual( {
						type: MessageType.Invocation, target: "test", arguments: [ 11 ],
					} );
				}
				finally
				{
		...
		it( "resends sent messages while disconnected on reconnect", async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				try
				{
					...
					expect( JSON.parse( connection.sentData[ 5 ] ) ).toEqual( {
						type: MessageType.Invocation, target: "test", arguments: [ 22 ],
					} );
				}
				finally
				{
		...
		it( "receiving ack removes buffered messages", async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				try
				{
					...
					expect( JSON.parse( connection.sentData[ 5 ] ) ).toEqual( {
						type: MessageType.Sequence, sequenceId: 3,
					} );
					expect( JSON.parse( connection.sentData[ 6 ] ) ).toEqual( {
						type: MessageType.Invocation, target: "test", arguments: [ 15 ],
					} );
				}
				finally
				{
		...
		it( "sends ack after receiving message", async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				try
				{
					...
					expect( JSON.parse( connection.sentData[ 2 ] ) ).toEqual( {
						type: MessageType.Ack, sequenceId: 1,
					} );
				}
				finally
				{
		...
		it( "sends ack after receiving many messages", async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				try
				{
					...
					expect( JSON.parse( connection.sentData[ 2 ] ) ).toEqual( {
						type: MessageType.Ack, sequenceId: 4,
					} );
				}
				finally
				{
		...
		it( "buffer full blocks sending, other sends also block promise but still send over connection", async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				try
				{
					...
					expect( connection.sentData.length ).toBe( 4 );
					expect( JSON.parse( connection.sentData[ 3 ] ) ).toEqual( {
						type: MessageType.Invocation, arguments: [ "x" ], target: "t",
					} );
