...
describe( "ServerSentEventsTransport", () =>
{
	it( "does not allow non-text formats", async () =>
	{
		...
		{
			const sse = new ServerSentEventsTransport( new TestHttpClient(), undefined, logger, { logMessageContent: true, EventSource: TestEventSource, withCredentials: true, headers: {} } );
	...
	[ [ "http://example.com", "http://example.com?access_token=secretToken" ], [ "http://example.com?value=null", "http://example.com?value=null&access_token=secretToken" ] ]
...
async function createAndStartSSE( logger: ILogger, url?: string, accessTokenFactory?: ( () => string | Promise<string> ), options?: IHttpConnectionOptions ): Promise<ServerSentEventsTransport>
{
	...
	const sse = new ServerSentEventsTransport( options?.httpClient || new TestHttpClient(), token, logger, { logMessageContent: true, EventSource: TestEventSource, withCredentials: true, timeout: 10205, ...options } );
