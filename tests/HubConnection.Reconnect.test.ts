	...
	it( "stop on close message if allowReconnect is missing and auto reconnect is enabled", async () =>
	{
		await VerifyLogger.run( async ( logger ) =>
		{
			...
			try
			{
				...
				connection.receive( {
					error: "Error!", type: MessageType.Close,
				} );
	...
	it( "can be stopped while restarting the underlying connection and negotiate throws", async () =>
	{
		await VerifyLogger.run( async ( logger ) =>
		{
			...
			const options: IHttpConnectionOptions = {
				WebSocket: TestWebSocket, httpClient: new TestHttpClient()
					.on( "POST", async () =>
					{
						++negotiateCount;
						if( negotiateCount === 1 )
			...
			expect( onreconnectedCount ).toBe( 0 );
			expect( closeCount ).toBe( 1 );
		}, "Failed to complete negotiation with the server: Error with negotiate", "Failed to start the connection: Error: Failed to complete negotiation with the server: Error with negotiate" );
	} );
} );
