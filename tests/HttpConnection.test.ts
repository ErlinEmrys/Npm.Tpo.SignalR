...
const defaultNegotiateResponse: INegotiateResponse = {
	availableTransports: [ { transport: "WebSockets", transferFormats: [ "Text", "Binary" ] }, { transport: "ServerSentEvents", transferFormats: [ "Text" ] }, { transport: "LongPolling", transferFormats: [ "Text", "Binary" ] } ], connectionId: defaultConnectionId, connectionToken: defaultConnectionToken, negotiateVersion: 1,
};
...
describe( "HttpConnection", () =>
{
	...
	it( "starting connection fails if getting id fails", async () =>
	{
		await VerifyLogger.run( async ( logger ) =>
		{
			const options: IHttpConnectionOptions = {
				...commonOptions, httpClient: new TestHttpClient()
		...
		}, "Failed to start the connection: Error: Failed to complete negotiation with the server: error", "Failed to complete negotiation with the server: error" );
	} );
	...
	it( "cannot start a running connection", async () =>
	{
		await VerifyLogger.run( async ( logger ) =>
		{
			const options: IHttpConnectionOptions = {
				...commonOptions, httpClient: new TestHttpClient()
					.on( "POST", () => defaultNegotiateResponse ), logger, transport: {
					connect()
					{
	...
	it( "can start a stopped connection", async () =>
	{
		await VerifyLogger.run( async ( logger ) =>
		{
			...
			await expect( connection.start( TransferFormat.Text ) )
				.rejects
				.toThrow( new Error( "Failed to complete negotiation with the server: reached negotiate." ) );
		}, "Failed to complete negotiation with the server: reached negotiate.", "Failed to start the connection: Error: Failed to complete negotiation with the server: reached negotiate." );
	} );
	...
	it( "all transport failure errors get aggregated", async () =>
	{
		await VerifyLogger.run( async ( loggerImpl ) =>
		{
			...
			const options: IHttpConnectionOptions = {
				WebSocket: TestWebSocket, ...commonOptions, httpClient: new TestHttpClient()
					.on( "POST", () =>
					{
						negotiateCount++;
				...
				logger: loggerImpl, transport: HttpTransportType.WebSockets,
			} as IHttpConnectionOptions;
			...
			await expect( startPromise )
				.rejects
				.toThrow( "Unable to connect to the server with any of the available transports. Error: WebSockets failed: Error: WebSocket failed to connect. " + "The connection could not be found on the server, either the endpoint may not be a SignalR endpoint, the connection ID is not present on the server, " + "or there is a proxy blocking WebSockets. If you have multiple servers check that sticky sessions are enabled. ServerSentEvents failed: Error: 'ServerSentEvents' is disabled by the client. LongPolling failed: Error: 'LongPolling' is disabled by the client." );
	...
	it( "negotiate called again when transport fails to start and falls back", async () =>
	{
		await VerifyLogger.run( async ( loggerImpl ) =>
		{
			let negotiateCount: number = 0;
			const options: IHttpConnectionOptions = {
				EventSource: ServerSentEventsNotAllowed, WebSocket: WebSocketNotAllowed, ...commonOptions, httpClient: new TestHttpClient()
					.on( "POST", () =>
					{
						negotiateCount++;
						return defaultNegotiateResponse;
				...
				logger: loggerImpl, transport: HttpTransportType.WebSockets | HttpTransportType.ServerSentEvents,
			} as IHttpConnectionOptions;
	...
	it( "failed re-negotiate fails start", async () =>
	{
		await VerifyLogger.run( async ( loggerImpl ) =>
		{
			...
			expect( negotiateCount ).toEqual( 2 );
		}, "Failed to start the transport 'WebSockets': Error: Don't allow Websockets.", "Failed to complete negotiation with the server: Error: negotiate failed", "Failed to start the connection: Error: Failed to complete negotiation with the server: Error: negotiate failed" );
	} );
		...
		await VerifyLogger.run( async ( logger ) =>
		{
			const options: IHttpConnectionOptions = {
				...commonOptions, httpClient: new TestHttpClient()
					...
					.on( "GET", () => { throw new Error( "fail" ); } ), logger,
			} as IHttpConnectionOptions;
		...
		await VerifyLogger.run( async ( logger ) =>
		{
			...
			const options: IHttpConnectionOptions = {
				...commonOptions, httpClient: new TestHttpClient()
					...
					.on( "GET", () => "" ), logger, transport: fakeTransport,
			} as IHttpConnectionOptions;
	...
	eachEndpointUrl( ( givenUrl: string, expectedUrl: string ) =>
	{
		it( `negotiate request for '${ givenUrl }' puts 'negotiate' at the end of the path`, async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				const options: IHttpConnectionOptions = {
					...commonOptions, httpClient: new TestHttpClient()
						...
						.on( "DELETE", () => new HttpResponse( 202 ) ), logger,
				} as IHttpConnectionOptions;
			...
			}, "Failed to complete negotiation with the server: Error: We don't care how this turns out: Status code '500'", "Failed to start the connection: Error: Failed to complete negotiation with the server: Error: We don't care how this turns out: Status code '500'" );
		} );
	} );
			...
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				const options: IHttpConnectionOptions = {
					...commonOptions, httpClient: new TestHttpClient()
						...
						.on( "GET", () => new HttpResponse( 204 ) ), logger, transport: requestedTransport,
				} as IHttpConnectionOptions;
				...
				await expect( connection.start( TransferFormat.Text ) )
					.rejects
					.toThrow( `Unable to connect to the server with any of the available transports. ${ negotiateResponse.availableTransports[ 0 ].transport } failed: Error: '${ negotiateResponse.availableTransports[ 0 ].transport }' is disabled by the client.` + ` ${ negotiateResponse.availableTransports[ 1 ].transport } failed: Error: '${ negotiateResponse.availableTransports[ 1 ].transport }' is disabled by the client.` );
		...
		it( `cannot be started if server's only transport (${ HttpTransportType[ requestedTransport ] }) is masked out by the transport option`, async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				const negotiateResponse = {
					availableTransports: [ { transport: "WebSockets", transferFormats: [ "Text", "Binary" ] }, { transport: "ServerSentEvents", transferFormats: [ "Text" ] }, { transport: "LongPolling", transferFormats: [ "Text", "Binary" ] } ], connectionId: "abc123",
				};
				...
				const options: IHttpConnectionOptions = {
					...commonOptions, httpClient: new TestHttpClient()
						.on( "POST", () => negotiateResponse )
						.on( "GET", () => new HttpResponse( 204 ) ), logger, transport: transportMask,
				} as IHttpConnectionOptions;
	...
	for( const [ val, name ] of [ [ null, "null" ], [ undefined, "undefined" ], [ 0, "0" ] ] )
	{
		it( `can be started when transport mask is ${ name }`, async () =>
		{
			...
			{
				const options: IHttpConnectionOptions = {
					WebSocket: websocket as any, ...commonOptions, httpClient: new TestHttpClient()
						.on( "POST", () => defaultNegotiateResponse )
						.on( "GET", () => new HttpResponse( 200 ) )
						.on( "DELETE", () => new HttpResponse( 202 ) ), logger, transport: val,
				} as IHttpConnectionOptions;
		...
		await VerifyLogger.run( async ( logger ) =>
		{
			const options: IHttpConnectionOptions = {
				WebSocket: websocket as any, ...commonOptions, httpClient: new TestHttpClient()
					...
					.on( "GET", () => { throw new Error( "Should not be called" ); } ), logger, skipNegotiation: true, transport: HttpTransportType.WebSockets,
			} as IHttpConnectionOptions;
	...
	it( "does not start non WebSockets transport if requested explicitly and skipNegotiation is true", async () =>
	{
		...
		{
			const options: IHttpConnectionOptions = {
				...commonOptions, httpClient: new TestHttpClient(), logger, skipNegotiation: true, transport: HttpTransportType.LongPolling,
			} as IHttpConnectionOptions;
		...
		await VerifyLogger.run( async ( logger ) =>
		{
			...
			const httpClient = new TestHttpClient()
				.on( "POST", /\/negotiate/, () =>
				{
					...
					return {
						availableTransports: [ { transport: "LongPolling", transferFormats: [ "Text" ] } ], connectionId: "0rge0d00-0040-0030-0r00-000q00r00e00",
					};
				} )
				.on( "GET", () =>
				{
			...
			const options: IHttpConnectionOptions = {
				...commonOptions, httpClient, logger, transport: HttpTransportType.LongPolling,
			} as IHttpConnectionOptions;
	...
	it( "redirects to url when negotiate returns it with access token", async () =>
	{
		await VerifyLogger.run( async ( logger ) =>
		{
			...
			const options: IHttpConnectionOptions = {
				...commonOptions, accessTokenFactory: () => "firstSecret", httpClient, logger, transport: HttpTransportType.LongPolling,
			} as IHttpConnectionOptions;
	...
	it( "authorization header removed when token factory returns null and using LongPolling", async () =>
	{
		await VerifyLogger.run( async ( logger ) =>
		{
			...
			const options: IHttpConnectionOptions = {
				...commonOptions, accessTokenFactory: () =>
				{
		...
		await VerifyLogger.run( async ( logger ) =>
		{
			...
			const transport: ITransport = {
				connect: ( url: string, transferFormat: TransferFormat ) =>
				{
					...
					return Promise.resolve();
				}, onclose: null, onreceive: null, send: ( data: any ) => Promise.resolve(), stop: () =>
				{
			...
			const options: IHttpConnectionOptions = {
				...commonOptions, httpClient: new TestHttpClient()
					.on( "POST", () => ( { connectionId: "42", availableTransports: [ availableTransport ] } ) ), logger, transport,
			} as IHttpConnectionOptions;
	...
	it( "missing negotiateVersion ignores connectionToken", async () =>
	{
		await VerifyLogger.run( async ( logger ) =>
		{
			...
			const options: IHttpConnectionOptions = {
				...commonOptions, httpClient: new TestHttpClient()
					.on( "POST", () => ( { connectionId: "42", connectionToken: "token", availableTransports: [ availableTransport ] } ) ), logger, transport,
			} as IHttpConnectionOptions;
	...
	it( "negotiate version 0 ignores connectionToken", async () =>
	{
		await VerifyLogger.run( async ( logger ) =>
		{
			...
			const options: IHttpConnectionOptions = {
				...commonOptions, httpClient: new TestHttpClient()
					.on( "POST", () => ( { connectionId: "42", connectionToken: "token", negotiateVersion: 0, availableTransports: [ availableTransport ] } ) ), logger, transport,
			} as IHttpConnectionOptions;
	...
	it( "negotiate version 1 uses connectionToken for url and connectionId for property", async () =>
	{
		await VerifyLogger.run( async ( logger ) =>
		{
			...
			const options: IHttpConnectionOptions = {
				...commonOptions, httpClient: new TestHttpClient()
					.on( "POST", () => ( { connectionId: "42", connectionToken: "token", negotiateVersion: 1, availableTransports: [ availableTransport ] } ) ), logger, transport,
			} as IHttpConnectionOptions;
	...
	it( "overwrites library headers with user headers on negotiate", async () =>
	{
		await VerifyLogger.run( async ( logger ) =>
		{
			...
			const options: IHttpConnectionOptions = {
				...commonOptions, headers, httpClient: new TestHttpClient()
					.on( "POST", ( r ) =>
					{
		...
		await VerifyLogger.run( async ( logger ) =>
		{
			...
			const options: IHttpConnectionOptions = {
				...commonOptions, httpClient: new TestHttpClient()
					...
					.on( "DELETE", () => new HttpResponse( 202 ) ), logMessageContent: true, logger: captureLogger, transport: HttpTransportType.LongPolling,
			} as IHttpConnectionOptions;
	...
	it( "send after restarting connection works", async () =>
	{
		...
		{
			const options: IHttpConnectionOptions = {
				...commonOptions, WebSocket: TestWebSocket, httpClient: new TestHttpClient()
					.on( "POST", () => defaultNegotiateResponse )
					.on( "GET", () => "" ), logger,
			} as IHttpConnectionOptions;
	...
	describe( ".constructor", () =>
	{
		...
		it( "uses EventSource constructor from options if provided", async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				const options: IHttpConnectionOptions = {
					...commonOptions, EventSource: TestEventSource as EventSourceConstructor, httpClient: new TestHttpClient().on( "POST", () =>
					{
						return {
							availableTransports: [ { transport: "ServerSentEvents", transferFormats: [ "Text" ] } ], connectionId: defaultConnectionId,
						};
					} ), logger, transport: HttpTransportType.ServerSentEvents,
				} as IHttpConnectionOptions;
			...
			}, "Failed to start the transport 'ServerSentEvents': Error: EventSource constructor called.", "Failed to start the connection: Error: Unable to connect to the server with any of the available transports. Error: ServerSentEvents failed: Error: EventSource constructor called." );
		} );
		...
		it( "uses WebSocket constructor from options if provided", async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				...
				const options: IHttpConnectionOptions = {
					...commonOptions, WebSocket: BadConstructorWebSocket as WebSocketConstructor, logger, skipNegotiation: true, transport: HttpTransportType.WebSockets,
				} as IHttpConnectionOptions;
	...
	describe( "startAsync", () =>
	{
		...
		it( "throws if trying to connect to an ASP.NET SignalR Server", async () =>
		{
			await VerifyLogger.run( async ( logger ) =>
			{
				const options: IHttpConnectionOptions = {
					...commonOptions, httpClient: new TestHttpClient()
						.on( "POST", () => "{\"Url\":\"/signalr\"," + "\"ConnectionToken\":\"X97dw3uxW4NPPggQsYVcNcyQcuz4w2\"," + "\"ConnectionId\":\"05265228-1e2c-46c5-82a1-6a5bcc3f0143\"," + "\"KeepAliveTimeout\":10.0," + "\"DisconnectTimeout\":5.0," + "\"TryWebSockets\":true," + "\"ProtocolVersion\":\"1.5\"," + "\"TransportConnectTimeout\":30.0," + "\"LongPollDelay\":0.0}" )
						.on( "GET", () => "" ), logger,
				} as IHttpConnectionOptions;
...
describe( "TransportSendQueue", () =>
{
	...
	it( "negotiate stateful reconnect false does not set query string", async () =>
	{
		await VerifyLogger.run( async ( logger ) =>
		{
			...
			const options: IHttpConnectionOptions = {
				...commonOptions, _useStatefulReconnect: false, WebSocket: TestWebSocket, httpClient: new TestHttpClient()
	...
	it( "negotiate Stateful Reconnect sets query string", async () =>
	{
		await VerifyLogger.run( async ( logger ) =>
		{
			...
			const options: IHttpConnectionOptions = {
				...commonOptions, _useStatefulReconnect: true, WebSocket: TestWebSocket, httpClient: new TestHttpClient()
	...
	it( "negotiate Stateful Reconnect does not work with long polling", async () =>
	{
		await VerifyLogger.run( async ( logger ) =>
		{
			...
			let firstPoll = true;
			const options: IHttpConnectionOptions = {
				...commonOptions, _useStatefulReconnect: true, transport: HttpTransportType.LongPolling, httpClient: new TestHttpClient()
					.on( "POST", () => negotiateResponse )
					.on( "GET", async () =>
					{
						if( firstPoll )
