...
describe( "LongPollingTransport", () =>
{
	...
	it( "sets timeout on requests", async () =>
	{
		await VerifyLogger.run( async ( logger ) =>
		{
			...
			const transport = new LongPollingTransport( httpClient, logger, { logMessageContent: false, withCredentials: true, headers: {}, timeout: 123 } );
	...
	it( "keeps user provided Authorization header when no accessTokenFactory", async () =>
	{
		await VerifyLogger.run( async ( logger ) =>
		{
			...
			const transport = new LongPollingTransport( httpClient, logger, {
				logMessageContent: false, withCredentials: true, headers: { Authorization: "Basic test" },
			} );
