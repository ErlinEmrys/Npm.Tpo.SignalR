...
import { HubConnectionState } from "../src/HubConnection";
...
const longPollingNegotiateResponse = {
	availableTransports: [ { transport: "LongPolling", transferFormats: [ "Text", "Binary" ] } ], connectionId: "abc123", connectionToken: "123abc", negotiateVersion: 1,
};
...
describe( "HubConnectionBuilder", () =>
{
		...
		await VerifyLogger.run( async ( logger ) =>
		{
			...
			const connection = createConnectionBuilder()
				.withUrl( "http://example.com", {
					...commonHttpOptions, httpClient: testClient, logger,
				} )
				.build();
	...
	describe( "configureLogging", () =>
	{
		...
		[ LogLevel.None, LogLevel.Critical, LogLevel.Error, LogLevel.Warning, LogLevel.Information, LogLevel.Debug, LogLevel.Trace ].forEach( ( minLevel ) =>
		{
		...
		it( "allows logger to be replaced", async () =>
		{
			...
			const connection = createConnectionBuilder( logger )
				.withUrl( "http://example.com", {
					...commonHttpOptions, httpClient: testClient,
				} )
				.build();
		...
		it( "does not replace HttpConnectionOptions logger if provided", async () =>
		{
			...
			const connection = createConnectionBuilder( hubConnectionLogger )
				.withUrl( "http://example.com", {
					httpClient: testClient, logger: httpConnectionLogger,
				} )
				.build();
	...
	it( "withAutomaticReconnect uses default retryDelays when called with no arguments", () =>
	{
		...
		{
			const retryContext = {
				previousRetryCount: retryCount++, elapsedMilliseconds: 0, retryReason: new Error(),
			};
