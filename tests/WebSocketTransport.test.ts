...
describe( "WebSocketTransport", () =>
{
	...
	it( "connect fails if there is error during connect", async () =>
	{
		await VerifyLogger.run( async ( logger ) =>
		{
			...
			await expect( connectPromise )
				.rejects
				.toThrow( "WebSocket failed to connect. The connection could not be found on the server, either the endpoint may not be a SignalR endpoint, " + "the connection ID is not present on the server, or there is a proxy blocking WebSockets. If you have multiple servers check that sticky sessions are enabled." );
			expect( connectComplete ).toBe( false );
		} );
	} );
	...
	[ [ undefined as any, undefined ], [ "secretToken", "Bearer secretToken" ], [ "", undefined ] ]
	...
	[ [ "http://example.com", "ws://example.com" ], [ "http://example.com?value=null", "ws://example.com?value=null" ], [ "https://example.com?value=null", "wss://example.com?value=null" ] ]
		.forEach( ( [ input, expected ] ) =>
		{
	...
	[ [ TransferFormat.Text, "send data" ], [ TransferFormat.Binary, new Uint8Array( [ 0, 1, 3 ] ) ] ]
		.forEach( ( [ format, data ] ) =>
		{
